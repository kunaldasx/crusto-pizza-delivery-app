...
const OrderPage = () => {
	...
	const handleCancelOrder = async () => {
		...
		const response = await updateOrderStatus(order._id, OrderStatus.Cancelled);
	...
	return (
		<section className="w-screen min-h-screen bg-section-background relative pt-20 sm:pt-30 md:pt-40 pb-20 px-6 md:px-20">
			<div className="w-full flex justify-between items-center">
				<h2 className="heading-2 mb-4">My Order</h2>
			</div>

			{!isLoadingOrder && order?.items.length ? (
				<div className="w-full bg-popover border-2 p-4 rounded-xl">
